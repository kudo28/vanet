...
simtime_t MyDecider80211p::processNewSignal(AirFrame* msg) {
...
int MyDecider80211p::getSignalState(AirFrame* frame) {
...
double MyDecider80211p::calcChannelSenseRSSI(simtime_t_cref start, simtime_t_cref end) {
...
void MyDecider80211p::calculateSinrAndSnrMapping(AirFrame* frame, Mapping **sinrMap, Mapping **snrMap) {
...
Mapping* MyDecider80211p::calculateNoiseRSSIMapping(simtime_t_cref start, simtime_t_cref end, AirFrame *exclude) {
...
DeciderResult* MyDecider80211p::checkIfSignalOk(AirFrame* frame) {
...
enum MyDecider80211p::PACKET_OK_RESULT MyDecider80211p::packetOk(double snirMin, double snrMin, int lengthMPDU, double bitrate) {
...
bool MyDecider80211p::cca(simtime_t_cref time, AirFrame* exclude) {
...
simtime_t MyDecider80211p::processSignalEnd(AirFrame* msg) {
	...
	if (result->isSignalCorrect()) {
		...
		if (frame->getUnderSensitivity()) {
			...
			if (((DeciderResult80211 *)result)->isCollision()) {
				phy->sendControlMsgToMac(new cMessage("Error", MyDecider80211p::COLLISION));
			}
			else {
...
void MyDecider80211p::setChannelIdleStatus(bool isIdle) {
...
void MyDecider80211p::changeFrequency(double freq) {
	centerFrequency = freq;
}

double MyDecider80211p::getCCAThreshold() {
	return 10 * log10(ccaThreshold);
}

void MyDecider80211p::setCCAThreshold(double ccaThreshold_dBm) {
	ccaThreshold = pow(10, ccaThreshold_dBm / 10);
}

void MyDecider80211p::switchToTx() {
...
MyDecider80211p::~MyDecider80211p() {}
;
