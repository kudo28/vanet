...
void MyPhyLayer80211p::initialize(int stage) {
	if (stage == 0) {
...
AnalogueModel* MyPhyLayer80211p::getAnalogueModelFromName(std::string name, ParameterMap& params) {
...
AnalogueModel* MyPhyLayer80211p::initializeLogNormalShadowing(ParameterMap& params) {
...
AnalogueModel* MyPhyLayer80211p::initializeJakesFading(ParameterMap& params) {
...
AnalogueModel* MyPhyLayer80211p::initializeBreakpointPathlossModel(ParameterMap& params) {
...
AnalogueModel* MyPhyLayer80211p::initializeTwoRayInterferenceModel(ParameterMap& params) {
...
AnalogueModel* MyPhyLayer80211p::initializeNakagamiFading(ParameterMap& params) {
...
AnalogueModel* MyPhyLayer80211p::initializeSimplePathlossModel(ParameterMap& params){
...
AnalogueModel* MyPhyLayer80211p::initializePERModel(ParameterMap& params) {
	double per = params["packetErrorRate"].doubleValue();
	return new PERModel(per);
}

Decider* MyPhyLayer80211p::getDeciderFromName(std::string name, ParameterMap& params) {
...
AnalogueModel* MyPhyLayer80211p::initializeSimpleObstacleShadowing(ParameterMap& params){
...
Decider* MyPhyLayer80211p::initializeDecider80211p(ParameterMap& params) {
	double centerFreq = params["centerFrequency"];
	MyDecider80211p* dec = new MyDecider80211p(this, sensitivity, ccaThreshold, allowTxDuringRx, centerFreq, findHost()->getIndex(), collectCollisionStatistics, coreDebug);
	dec->setPath(getParentModule()->getFullPath());
	return dec;
}

void MyPhyLayer80211p::changeListeningFrequency(double freq) {
	MyDecider80211p* dec = dynamic_cast<MyDecider80211p*>(decider);
	assert(dec);
	dec->changeFrequency(freq);
}
void MyPhyLayer80211p::handleSelfMessage(cMessage* msg) {
...
AirFrame *MyPhyLayer80211p::encapsMsg(cPacket *macPkt)
{
...
int MyPhyLayer80211p::getRadioState() {
...
void MyPhyLayer80211p::setCCAThreshold(double ccaThreshold_dBm) {
	ccaThreshold = pow(10, ccaThreshold_dBm / 10);
	((MyDecider80211p *)decider)->setCCAThreshold(ccaThreshold_dBm);
}
double MyPhyLayer80211p::getCCAThreshold() {
